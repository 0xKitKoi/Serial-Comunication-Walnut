...
void NerveSystemLayer::OnUIRender() {
    ImGui::Begin("NERVE CONNECTION STATUS");
    DrawEVAGlowBorder();
...
EvaSignalConnector::DrawSignalRow(dl, "CTS",  "00132", m_CtsSignal.isConnected,
    false, cursor, winW, 0, 6);
cursor = ImGui::GetCursorScreenPos();
EvaSignalConnector::DrawSignalRow(dl, "DSR",  "00133", m_DsrSignal.isConnected,
    false, cursor, winW, 1, 6);
cursor = ImGui::GetCursorScreenPos();
EvaSignalConnector::DrawSignalRow(dl, "DCD",  "00134", m_DcdSignal.isConnected,
    false, cursor, winW, 2, 6);
cursor = ImGui::GetCursorScreenPos();
EvaSignalConnector::DrawSignalRow(dl, "RING", "00135", m_RingSignal.isConnected,
    false, cursor, winW, 3, 6);
...
cursor = ImGui::GetCursorScreenPos();
EvaSignalConnector::DrawSignalRow(dl, "DTR",  "00201", m_DtrControl.isConnected,
    true, cursor, winW, 4, 6);
    // Handle DTR click
    if (ImGui::IsItemClicked() && m_PortNumber >= 0) {
        m_DtrControl.isConnected = !m_DtrControl.isConnected;
        m_DtrControl.isConnected ? RS232_enableDTR(m_PortNumber) : RS232_disableDTR(m_PortNumber);
    }

cursor = ImGui::GetCursorScreenPos();
EvaSignalConnector::DrawSignalRow(dl, "RTS",  "00202", m_RtsControl.isConnected,
    true, cursor, winW, 5, 6);
    if (ImGui::IsItemClicked() && m_PortNumber >= 0) {
        m_RtsControl.isConnected = !m_RtsControl.isConnected;
        m_RtsControl.isConnected ? RS232_enableRTS(m_PortNumber) : RS232_disableRTS(m_PortNumber);
    }
    ...
    bool allConnected = m_CtsSignal.isConnected && m_DsrSignal.isConnected;
    if (allConnected)
        ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.4f, 1.0f), "ALL NERVE CONNECTIONS INTACT");
    else
        ImGui::TextColored(ImVec4(1.0f, 0.2f, 0.2f, 1.0f), "NERVE CONNECTIONS SEVERED");
